...
function openWavePaymentModal(order, paymentData) {
    ...
    const launchUrl = paymentData.launchUrl || `https://pay.wave.com/m/M_ci_vBawLmucg2kQ/c/ci/?amount=${grandTotal}&client_reference=${encodeURIComponent(order.id)}`;
...
function openOperatorPaymentModal(op) {
    ...
    const officialWaveUrl = `https://pay.wave.com/m/M_ci_vBawLmucg2kQ/c/ci/?amount=${grandTotal}&client_reference=${encodeURIComponent(orderId)}`;
